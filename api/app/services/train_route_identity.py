    ...
    *,
    ...
    for record in _dedupe_train_records(first_records):
        ...
        if actual_departure < ready_minute:
            continue
        # For ordinary same-day route steps, a train that departs after the
        # route's stated arrival time cannot be the train represented by the
        # step. Excluding it prevents unrelated late-night runs from poisoning
        # an afternoon candidate while leaving overnight behavior unchanged.
        if route_arrival_minute >= ready_minute and actual_departure > route_arrival_minute:
            continue
