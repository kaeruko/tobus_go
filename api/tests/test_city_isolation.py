...
class CityIsolationTest(unittest.TestCase):
    ...
    def test_yokohama_is_a_supported_isolated_backend_city(self):
        app = FastAPI()
        install_city_isolation(app, "yokohama")

        @app.get("/probe")
        async def probe():
            return {"city": "yokohama"}

        client = TestClient(app)
        matching = client.get("/probe", headers={"X-App-City": "yokohama"})
        self.assertEqual(matching.status_code, 200)
        self.assertEqual(matching.json(), {"city": "yokohama"})

        mismatch = client.get("/probe", headers={"X-App-City": "tokyo"})
        self.assertEqual(mismatch.status_code, 409)
        self.assertEqual(mismatch.json()["detail"]["code"], "city_mismatch")
