...
ENGINE_PATH = Path("api/toei_engine.py")
TEST_PATH = Path("api/tests/test_route_search_regression.py")
...
def patch_engine() -> None:
    ...
    old_dispatch = (
        "        path_gen = find_paths_generator(G, tm, a_phys, target_node, start_time, "
        "day_type=day_type, max_search=30000, max_visited=100000, "
        "max_travel_min=MAX_TRAVEL_MIN, delays_snapshot=delays_snapshot, "
        "virtual_dest_connections=virtual_dest_connections, target_coords=target_coords)\n"
    )
    dispatch_count = engine.count(old_dispatch)
    if dispatch_count != 1:
        raise RuntimeError(
            "expected exactly one legacy comfort dispatch; "
            f"found {dispatch_count}"
        )

    new_dispatch = (
        '        if mode == "fewTransfers":\n'
        "            path_gen = find_few_transfers_paths_generator(G, tm, a_phys, "
        "target_node, start_time, day_type=day_type, max_search=30000, "
        "max_visited=100000, max_travel_min=MAX_TRAVEL_MIN, "
        "delays_snapshot=delays_snapshot, "
        "virtual_dest_connections=virtual_dest_connections, "
        "target_coords=target_coords)\n"
        "        else:\n"
        "            path_gen = find_paths_generator(G, tm, a_phys, target_node, "
        "start_time, day_type=day_type, max_search=30000, max_visited=100000, "
        "max_travel_min=MAX_TRAVEL_MIN, delays_snapshot=delays_snapshot, "
        "virtual_dest_connections=virtual_dest_connections, "
        "target_coords=target_coords)\n"
    )
    engine = engine.replace(old_dispatch, new_dispatch, 1)

    marker = "\ndef find_fastest_path("
    marker_count = engine.count(marker)
    if marker_count != 1:
        raise RuntimeError(
            "expected exactly one find_fastest_path marker; "
